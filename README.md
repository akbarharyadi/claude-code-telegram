...
REVIEW_PARALLEL=3                          # reviews (or diff parts) in flight at once
```
...
- **Binary verdicts** — every review files either an **approval** or a **request for changes**; there is no fence-sitting "comment". "I could not verify this" *is* a request for changes, with the reason in the summary. A diff past GitHub's 20k-line API cap files request_changes marked `needs_human` and is never auto-approved at the round limit.
- **Verification pass** (`REVIEW_VERIFY`, on) — before a request_changes is filed, a second pass re-checks each claimed defect against the code; claims that do not survive are dropped. If every claim falls, the review still files request_changes and says so plainly — not confident enough to approve means changes requested.
- **Coverage gate** (`REVIEW_COVERAGE_GATE`, on) — an approval whose findings never mention some touched files gets one focused remediation pass over exactly those files ("read them now, add findings"). If the gap closes, the approval stands; if not, it files request_changes naming the files. Lockfiles and build artifacts are exempt; auto-approvals from the changes-limit are left alone.
